...
AS $$
  SELECT EXISTS (
    -- Cast enum -> text to avoid "unsafe use of new value" within the same transaction
    -- right after ALTER TYPE ... ADD VALUE.
    SELECT 1 FROM public.user_roles WHERE user_id = _user_id AND role::text IN ('admin', 'super_admin', 'asesor')
  )
$$;
